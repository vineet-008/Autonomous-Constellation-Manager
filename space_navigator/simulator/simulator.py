...
from numpy_orbit import plot_orbit_3d as plot_planet
...
def draw_sphere(axis, centre, radius, wireframe_params={}):
    ...
    """
    import os
    if os.path.exists("data/environments/earth.jpg"):
        try:
            from PIL import Image
            # Resize appropriately matching u, and v matrix resolutions
            img = Image.open("data/environments/earth.jpg").resize((50, 100))
            facecolors = np.array(img) / 255.0
            
            u, v = np.mgrid[0:2 * np.pi:100j, 0:np.pi:50j]
            x = radius * np.cos(u) * np.sin(v) + centre[0]
            y = radius * np.sin(u) * np.sin(v) + centre[1]
            z = radius * np.cos(v) + centre[2]
            return axis.plot_surface(x, y, z, facecolors=facecolors, shade=False)
        except Exception as e:
            pass # Fall back to wireframe if texture overlay algorithm fails
...
class Simulator:
    ...
    def print_end(self, simulation_time):
        ...
        df["threshold"] = df["threshold"].astype(object).fillna(value="not taken")
